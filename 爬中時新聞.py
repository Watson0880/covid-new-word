...
point_word = "新冠肺炎"
cnews_url = "https://www.chinatimes.com"
url = "https://www.chinatimes.com/search/"
page = 1
url1 = "?page="
titlepath = 'ctnewstitle.txt'
    ...
    for point in seed:
        ...
        for i in range(2000,2500):
            ...
            for j in range(0,len(news_list)):
                #print(news_list[j].select('a')[0].text)
                #print(news_list[j].select('a')[0].get("href"))
                
                if news_dict.get(news_list[j].select('a')[0].text) != None: #確認是否找過了
                    continue
                time.sleep(1)
                print("之",j)
                new_url = news_list[j].select('a')[0].get("href")
                ...
                with open(titlepath,'a', encoding='UTF-8') as f:
                    f.write(news_list[j].select('a')[0].text+'\n')  #寫入title
                    news_dict[news_list[j].select('a')[0].text] = 1
